...
array=( HOST_CORPORA_OUT HOST_TRAIN TRAIN_DATA TRAIN_LOGDIR HOST_PORT_NOTEBOOK HOST_PORT_TENSORBOARD CPU_SET NV_GPU MODEL_TYPE DEV_FILE_INDEX BATCH_SIZE TREE_EMBEDDER LEARNING_RATE OPTIMIZER EARLY_STOP_QUEUE ROOT_FC_SIZE LEAF_FC_SIZE STATE_SIZE KEEP_PROB INIT_ONLY CONCAT_MODE MAX_DEPTH CONTEXT NEG_SAMPLES NO_FIXED_VECS TRAIN_FILES )
echo ".env vars:"
for i in "${array[@]}"
do
	if [ -z "${!i}" ]; then
