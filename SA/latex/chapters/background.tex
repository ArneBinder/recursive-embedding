...
\begin{figure}[htb!]
  ...
  \caption{Dependency parse tree with typed relations. Arcs point from heads to their dependents and are labeled with dependency types.}
...
%In addition to 
A head with its dependents marks a linguistic unit in the hierarchical structure, similar to a linguistic constituent. In addition to this structural \todo{hierarchical?} information, dependency grammars allow to classify the kinds of dependency relations in terms of the grammatical role the dependent plays with respect to its head, e.g. \textit{subject}, \textit{direct object} or \textit{adverbial modifier}.

%Grammatical roles or grammatical functions describe relationships 
