...
Finding these contradictory results with respect to the applied measure, we took a closer look at the deviations of the predicted relatedness scores from the gold scores. Figure~\ref{fig:fig_sorted_errors_sep} shows the deviations per setting ordered by gold score. It suggests that the averaging model (\texttt{order aware} = NO) strongly overestimates the relatedness score. In fact, the mean deviation for all predictions in the averaging case is significant ($t < 0.0001$) higher ($0.085$) when compared with non-averaging ($0.019$). Such a bias distorts the Pearson'r as it ...

% One explanation ... MSE was used as cost function while optimization. NO sense
...
\begin{figure}[htb!]
	...
	\caption{Deviations of the predictions from gold similarity scores by setting (<dependency available>, <order aware>). The averaging model (order aware = NO; plots at the left) clearly overestimates.}
