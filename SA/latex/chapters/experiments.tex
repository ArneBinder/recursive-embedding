...
Finding these contradictory results\todo{AB:with respect to the applied measure?}, we took a closer look at the deviations of the predicted relatedness scores from the gold scores. Figure~\ref{fig:fig_sorted_errors_sep} shows the deviations per setting ordered by gold score. It suggests that the averaging model (\texttt{order aware} = NO) strongly overestimates the relatedness score. In fact, the mean deviation for all predictions in the averaging case ($+0.085$) is significant ($t < 0.0001$) higher when compared with non-averaging ($+0.019$). Because such a bias distorts the Pearson'r and \ac{MSE} was used as cost function while training, we focus at the \ac{MSE} measure for the rest of this work.
