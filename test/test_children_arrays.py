...
def test_concat_extend(forest1):
    lexicon = forest1.lexicon
    ...
    #test_reverse(forest4)
    print('')


def test_reverse(forest):
    forest.children_array_to_dict(clear_arrays=True)
    roots = forest.roots
    forest.reset_cache_values()
    desc_indices_r0 = sorted(forest.descendant_indices(roots[0]))
    d = forest._children_dict
    forest.visualize(filename='test_dict.svg')
    print('')
...
if __name__ == '__main__':
    ...
    forest = Forest(data=data, parents=parents, lexicon=lexicon)
    forest.visualize(filename='test.svg')
    forest.children_dict_to_arrays()
    #forest.reset_cache_values()
    forest.visualize(filename='test_arrays.svg')

    forest2 = Forest(data=data, parents=parents, lexicon=lexicon)
    forest2.children_dict_to_arrays()

    forest3 = Forest.concatenate([forest, forest2])
    for i, tree in enumerate(forest3.trees()):
        _data = tree[0]
        _parents = tree[1]
        Forest(data=_data, parents=_parents, lexicon=lexicon).visualize('test_tree_%i' % i)
    #forest.children_array_to_dict()
    #test_reverse(forest)
    #test_concat_extend(forest)
