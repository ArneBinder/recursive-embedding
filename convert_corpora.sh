...
## convert SICK
python src/corpus_rdf.py CONVERT -i "$CORPORA_OUT"/SICK_RDF/spacy -c sck:vocab#entailment_judgment -g "$GLOVE_TXT" -m 2
python src/corpus_rdf.py CONVERT -i "$CORPORA_OUT"/SICK_RDF/corenlp -c sck:vocab#entailment_judgment -g "$GLOVE_TXT" -m 2
...
# entity masking; entity-to-entity spans
python src/corpus_rdf.py CONVERT -i "$CORPORA_OUT"/TACRED_RDF/None -c tac:vocab#relation -g "$GLOVE_TXT" -m 2 -t -s
# entity masking; re-linked entities; link via deprel
python src/corpus_rdf.py CONVERT -i "$CORPORA_OUT"/TACRED_RDF/None -c tac:vocab#relation -g "$GLOVE_TXT" -m 2 -t -l -e
